...
namespace Core
{
	 ...
	 */
	template<Movable K, Movable V, Hasher<K> H = Hash<K>, EqualsComparator<K> C = DefaultComparator<K>, bool IsMultiMap = false>
	class HashMap
	{
	private:
		struct Node
	...
	public:
		 ...
		 */
		template<ForwardIterator It>
		explicit HashMap(const It& begin, const It& end, usize minBuckets, H hasher, C comp, Alloc::IAllocator& alloc = g_GlobalAlloc) noexcept requires CopyConstructable<K>&& CopyConstructable<V>;

		/**
		 * \brief Create a HashMap with the contents of another HashMap
		 * \param[in] other HashMap to copy
		 */
		HashMap(const HashMap& other) noexcept requires CopyConstructable<K>&& CopyConstructable<V>;
		/**
		 * \brief Create a HashMap with the contents of another HashMap, but with a different allocator
		 * \param[in] other HashMap to copy
		 * \param[in] alloc Allocator the container should use
		 */
		HashMap(const HashMap& other, Alloc::IAllocator& alloc) noexcept requires CopyConstructable<K>&& CopyConstructable<V>;
		/**
		...
		auto operator=(const InitializerList<Pair<K, V>>& il) noexcept -> HashMap requires CopyConstructable<K>&& CopyConstructable<V>;
		auto operator=(const HashMap& other) noexcept -> HashMap requires CopyConstructable<K>&& CopyConstructable<V>;
