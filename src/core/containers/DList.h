...
namespace Core
{
	...
	public:
		 ...
		 */
		explicit DList(usize count, Alloc::IAllocator& alloc = g_GlobalAlloc) noexcept;
		/**
		 ...
		 * \param alloc Allcoator the container shoud use
		 */
		explicit DList(usize count, const T& val, Alloc::IAllocator& alloc = g_GlobalAlloc) noexcept;
		 ...
		 */
		explicit DList(const InitializerList<T>& il, Alloc::IAllocator& alloc = g_GlobalAlloc) noexcept;
		/**
		...
		explicit DList(const It& begin, const It& end, Alloc::IAllocator& alloc = g_GlobalAlloc) noexcept;
		/**
		...
		auto operator=(const InitializerList<T>& il) noexcept -> DList<T>&;
		auto operator=(const DList<T>& other) noexcept -> DList<T>&;
		 ...
		 */
		template<ForwardIterator It>
		auto Insert(const ConstIterator& it, const It& begin, const It& end) noexcept -> Iterator;
		/**
		 ...
		 * \return Iterator to the first element that was inserted
		 */
		auto Insert(const ConstIterator& it, const InitializerList<T>& il) noexcept -> Iterator;
		/**
