	...
	constexpr auto Pack<T, Width>::And(const Pack& other) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				...
				{
					pack.data.m128[0] = HalfPack(0).And(other.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).And(other.HalfPack(1)).data;
					return pack;
				}
		...
		for (usize i = 0; i < Width; ++i)
			pack.data.bits[i] = data.bits[i] & other.data.bits[i];
		return pack;
	}
	...
	constexpr auto Pack<T, Width>::AndNot(const Pack& other) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).AndNot(other.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).AndNot(other.HalfPack(1)).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::Xor(const Pack& other) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).Xor(other.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).Xor(other.HalfPack(1)).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::Or(const Pack& other) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).Or(other.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).Or(other.HalfPack(1)).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::Not() const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL 
		{
			if constexpr (IsNative())
				return Xor(Pack::Ones());
		}

		for (usize i = 0; i < Width; ++i)
			pack.data.bits[i]  = ~data.bits[i];
		return pack;
	}
	...
	constexpr auto Pack<T, Width>::Blend(const Pack& other, const Pack& mask) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			if constexpr (Is128Bit())
			{
				...
				if constexpr (IsNative())
				{
					return AndNot(mask).Or(other.And(mask));
				}
			}
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).Blend(other.HalfPack(0), mask.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).Blend(other.HalfPack(1), mask.HalfPack(1)).data;
					return pack;
				}
				...
				{
#if HAS_AVX
					pack.data.sse_m256d = _mm256_blendv_pd(data.sse_m256d, other.data.sse_m256d, mask.data.sse_m256d);
					return pack;
#endif
				}
				else if constexpr (IsF32<T>)
				{
#if HAS_AVX
					pack.data.sse_m256 = _mm256_blendv_ps(data.sse_m256, other.data.sse_m256, mask.data.sse_m256);
					return pack;
#endif
				}
				else
				{
#if HAS_AVX2
					pack.data.sse_m256i = _mm256_blendv_epi8(data.sse_m256i, other.data.sse_m256i, mask.data.sse_m256i);
					return pack;
#endif
				}
			}
		...
		for (usize i = 0; i < Width; ++i)
			pack.data.raw[i] = mask.data.bits[i] ? other.data.raw[i] : data.raw[i];
		return pack;
	}
	...
	constexpr auto Pack<T, Width>::ShiftL(const Pack<U, Width>& count) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).ShiftL(count.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).ShiftL(count.HalfPack(1)).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftL(U count) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).ShiftL(count).data;
					pack.data.m128[1] = HalfPack(1).ShiftL(count).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftL() const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).template ShiftL<Count>().data;
					pack.data.m128[1] = HalfPack(1).template ShiftL<Count>().data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftRA(const Pack<U, Width>& count) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).ShiftRA(count.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).ShiftRA(count.HalfPack(1)).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftRA(U count) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).ShiftRA(count).data;
					pack.data.m128[1] = HalfPack(1).ShiftRA(count).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftRA() const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).template ShiftRA<Count>().data;
					pack.data.m128[1] = HalfPack(1).template ShiftRA<Count>().data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftRL(const Pack<U, Width>& count) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).ShiftRL(count.HalfPack(0)).data;
					pack.data.m128[1] = HalfPack(1).ShiftRL(count.HalfPack(1)).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftRL(U count) const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).ShiftRL(count).data;
					pack.data.m128[1] = HalfPack(1).ShiftRL(count).data;
					return pack;
				}
	...
	constexpr auto Pack<T, Width>::ShiftRL() const noexcept -> Pack
	{
		...
		IF_NOT_CONSTEVAL
		{
			...
			else if constexpr (Is256Bit())
			{
				if constexpr (IsNative() && !IsNative256())
				{
					pack.data.m128[0] = HalfPack(0).template ShiftRL<Count>().data;
					pack.data.m128[1] = HalfPack(1).template ShiftRL<Count>().data;
					return pack;
				}
